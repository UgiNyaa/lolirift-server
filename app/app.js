...
wss.on('connection', (ws) => {
  ...
  player = world.players.find((p) => p.name === ws.upgradeReq.headers.name)
  ...
  ws.on('message', (message) => {
    ...
    if (unit.owner !== player.id) {
      ws.send({ action: 'error', message: 'you do not won this unit' })
      return
    }

    // checking whether the unit can be manipulated with this action
    if (!unit.actions.includes(json.action)) {
      ws.send({ action: 'error', message: 'this unit can not use the action: ' + json.action })
    }
